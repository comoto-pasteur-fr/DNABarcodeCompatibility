...
if((length(argv) < 2) | (length(argv) > 7)){
  ...
  inputfile=normalizePath(argv[7])
}


## Main ####
dna_barcode_set <- local(get(load(inputfile)))

nb_total_combinations <- as.numeric(choose(barcode_number, mplex_level))
index <- dplyr::sample_n(dna_barcode_set, nrow(dna_barcode_set), replace = FALSE)
