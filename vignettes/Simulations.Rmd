---
title: "Simulations"
author: "Trébeau, Boutet de Monvel, et al., Etournay."
...
* Step2: optimizing the selection of compatible barcodes to minimize heterogeneity of barcode usage
    + Step2a: optimization by a greedy search method without iteration
    + Step2b: optimization by R repetitions of a greedy search method


# Illumina barcode set from TruSeq small RNA kits
## Finding compatible combinations of barcodes: simulations relative to Step1 of the workflow
...
## Analyzing barcode-set properties: set parameters for the simulations
...
### Running simulations for exhaustive searches
...
inputfile <- tempfile()
df <- DNABarcodeCompatibility::IlluminaIndexes
save(df, file=inputfile)
...
The execution time of an exhaustive search increases linearly with the total number ${n \choose k}$ of combinations. The constant of proportionality corresponds to the average computation time required to check compatibility of a given combination. This time is slighty faster for the 4-channel chemistry. 
For the ease of use, we fix to a few seconds the time of execution. On a standard computer, this corresponds to an exhaustive search among 2000 combinations for the 4-channel chemistry.
Above 2000 combinations, the program will perform a search of compatible barcodes from a randomly generated set of barcode combinations. Other important aspects also motivate this choice (see below).
...
The total number of combinations (compatible or not) is given by the binomial coefficient ${n \choose k}$ where $n$ is the number of barcodes in the dataset, referred to as the barcode-set size, and $k$ is the mutliplex level (where $n$ and $k$ being positive integers).
...
Therefore, in these conditions, it is advantageous to perform an exhaustive search of compatible combinations for a multiplex level of 2. For a multiplex level of 4 or higher, the program will typically perform a search of compatible barcodes from a randomly generated set of barcode combinations.
...
### Probability to find compatible barcodes in the total ensemble of combinations
...
ggtitle(paste("Average fraction of compatible combinations ( average over", R,"simulations )"))
...
The frequencies of the various barcodes appearing in the compatible combinations generated above may be highly heterogeneous for small multiplex levels. 
We plot here the distribution of barcodes averaged over the different barcode sets, for distinct chemistries and multiplex levels.
Despite the averaging of the distibutions over different barcode sets of different sizes, the heterogeneity between barcode frequencies is clearly apparent.
...
## Optimizing combinations: set parameters for simulations relative to Step2 of the workflow

* Step2: optimize the set of compatible barcodes to minimize the heterogeneity of barcode frequencies
...
**Step2a:** find the probability $P_{1}$ of to get an optimal combination given the subset size, and estimate $R$ for $P_{R} \geq 0.95$, in order to refine the optimization in a reasonable computation time.


**Step2b:** check the efficacy of the optimizer function on a few challenging examples


### Step2a: table of parameters for the simulations
...
#### Running simulations for step2a (without iteration)
...
# Define barcode set
barcode_set = DNABarcodeCompatibility::IlluminaIndexes
...
if (F) {
  system.time({
    for (i in L) {
      ...
      index <- dplyr::sample_n(barcode_set, nrow(barcode_set), replace = FALSE)
      for (j in N) {
        for (k in C) {
...
We perform `r R` repetitions of the greedy search for the conditions (described in table 2.3.1 above). When the set of compatible combinations is large, the computation time, required to run these greedy algorithms on the full set of compatible combinations generated in step1, may become prohibitively large. To reduce the computation time, we apply the greedy search on a randomly generated subset of compatible combinations of a size comprized between `r min(L)` and `r max(L)`. Here, we want to know how the size of the subset influences the computation time for each of the greedy search algorithm.
...
We perform `r R` repetitions of the greedy search for the conditions (see table 2.3.1 above) to estimate the probability to reach the theoretical maximum entropy $S_{max}$ in each condition. Here, we test how the size of the subset influences the probability to reach $S_{max}$.
...
#### Estimation of the maximum execution time (R iterations) of a greedy search to obtain the maximum entropy selection in 95% of cases
...
sim1_probaSmax %>% 
  ...
  mutate(mplex_level=as.factor(mplex_level),
         ...
         nb_lane=paste(nb_lane,"lanes"),
         barcode_number=paste("set of",barcode_number,"barcodes")) %>%
  mutate(R=ceiling(log(1-P)/log(1-ProbaOfSsup)),
         exec_time=(R+1)*time.avg) %>%
  ggplot(aes(x = thrs_size_comb, y = exec_time, color=R, shape=mplex_level)) +
  geom_point(size = 2) +
...
* Max execution time / iteration number as a function of the number of lanes $a$, grouped by algorithm, multiplex level, and number of compatible barcodes used for the search

Data are now pooled regardless chemistry and barcode-set sizes. In the graphs below, the number displayed in the right grey panel represents the size the randomly chosen subset of compatible combinations. We confirm that the greedy-descent search performs faster for small subsets of compatible combinations. For subsets larger than 30 compatible combinations, it seems that 50 iterations would be a reasonable number for an efficient and fast optimization by the greedy-exchange search.
...
dat %>%
  ggplot(aes(x = as.factor(nb_lane), y = R, color=mplex_level)) + 
  geom_boxplot() +
...
dat %>% group_by(thrs_size_comb, nb_lane, mplex_level, barcode_number, chemistry) %>%
  mutate(GD_ov_GE_time_ratio= exec_time[1]/exec_time[2]) %>% ungroup() %>% filter(thrs_size_comb<400) %>%
  ggplot(aes(x = as.factor(nb_lane), y = GD_ov_GE_time_ratio, color=mplex_level)) +
  geom_boxplot() +
  scale_y_log10(breaks=c(0.1, 0.5, 1, 5, 10, 50, 100)) +
  # scale_y_log10() +
  ...
  # facet_grid(thrs_size_comb~algorithm) +
  facet_grid(thrs_size_comb~.) +
  ggtitle(paste("Relative time performance of the greedy algorithms to find a best optimized solution in",P*100,"% of cases")) +
  xlab("Number of lanes") +
  ylab("Time ratio of greedy-descent over greedy-exchange execution (log scale)")
...
### Step2b: table of parameters used for the simulations
...
#### Running simulations for step2b with iterations
...
We compare the distributions of entropy values obtained using 3 different strategies over a maximum of `r max_iteration` iterations per trial. We performed `r R` trials for each strategy. We calculate and compare the 3 resulting entropy values:

1. **S_random**: no optimization; a subset of compatible combinations is randomly picked up only once (no iteration)
2. **S_opt**: a subset of compatible combinations is optimized using the greedy-exchange algorithm, stopping when a selection of entropy $S_{max}$ is obtained, or when the iteration number reaches `r max_iteration`.
...
In the graphs below, the blue line indicates the position of the theoretical value of the maximum attainable entropy $S_{max}$. Solid lines indicate that $S_{max}$ has not been reached over `r max_iteration` iterations. Either the probability to find $S_{max}$ is too low to be reached in `r max_iteration` iterations, or  $S_{max}$ cannot be reached at all due to constraints in the barcode set. Furthermore, the dashed line indicates that $S_{max}$ has been reached.
...
#### Comparison of barcode distributions between a randomly generated set and an optimized selection of compatible combinations
...
We use the same experimental conditions as above for a fair comparison. Here, the optimal selection has a uniform distribution of barcodes (of entropy $S_{max} = log(N)$ where $N$ is the number of libraries, and $N=a.k$ where $a$ is the number of lanes and $k$ the multiplex level), and is reached by the greedy algorithms.
...
Here, a selection with uniform barcode distribution cannot be reached because $N=a.k$ is greater than, but nut divisible by the number of available barcodes $n$. However, the selection having the maximum possible entropy $S_{max}$ is reached.
...
# Randomly-generated barcode set from the DNABarcodes package
...
# Define Illumina TruSeq small RNA barcode set
inputfile <- tempfile()
save(newBarcodeSet, file=inputfile)
...
results2 %>% group_by(mplex_level, barcode_number, chemistry) %>%
  ...
  xlab("Total number of combinations [log scale]")+
  ylab("Number of compatible combinations [log scale]")+
  ggtitle(paste("Number of compatible combinations"))
...
## Optimizing combinations: set parameters for the simulations
...
N = 3:5
C = c(2,4)
thrs_size_comb_for_GD=c()
thrs_size_comb_for_GE=c(seq(25,50,5),seq(60,100, 10), seq(150,300,50))
max_iteration = 0
nb_lane=c(4,6,8)
algorithm= c("greedy_exchange")
...
### Running simulations for the optimization without iteration
...
if (F) {
  system.time({
    ...
    sim3_barcodeStat_random <- read.table(file=outputfile1, header = T, sep = " ", stringsAsFactors = FALSE)
    save(sim3_barcodeStat_random, file = paste0(SCRIPT_DIR,"sim3_barcodeStat_random"))
    
    sim3_barcodeStat_opt <- read.table(file=outputfile2, header = T, sep = " ", stringsAsFactors = FALSE)
    save(sim3_barcodeStat_opt, file = paste0(SCRIPT_DIR,"sim3_barcodeStat_opt"))
    
    sim3_probaSmax <- read.table(file=outputfile3, header = T, sep = " ", stringsAsFactors = FALSE)
    save(sim3_probaSmax, file = paste0(SCRIPT_DIR,"sim3_probaSmax"))
    
    sim3_barcodeStat_random_opt <- read.table(file=outputfile4, header = T, sep = " ", stringsAsFactors = FALSE)
    save(sim3_barcodeStat_random_opt, file = paste0(SCRIPT_DIR,"sim3_barcodeStat_random_opt"))
  })
}


# Import results
sim3_barcodeStat_random <- local(get(load(paste0(SCRIPT_DIR,"sim3_barcodeStat_random"))))
sim3_barcodeStat_opt <- local(get(load(paste0(SCRIPT_DIR,"sim3_barcodeStat_opt"))))
# sim3_barcodeStat_random_opt <- local(get(load(paste0(SCRIPT_DIR,"sim3_barcodeStat_random_opt"))))
sim3_probaSmax <- local(get(load(paste0(SCRIPT_DIR,"sim3_probaSmax"))))
```
...
```{r echo = FALSE,  warning=FALSE, message=F, fig.width=11, fig.height=22, eval=T}

sim3_probaSmax %>% group_by(chemistry, barcode_number, mplex_level, nb_lane, algorithm) %>%
  filter(thrs_size_comb<nb_comp_combinations) %>%
  group_by(chemistry, barcode_number, mplex_level, nb_lane, algorithm, thrs_size_comb) %>%
  mutate(S_sup=max(S_opt),
         isSsup=ifelse(abs(S_opt-S_sup)<10^(-10), T, F)) %>%
...
```{r echo = FALSE,  warning=FALSE, fig.width=11, fig.height=22, eval=T}
# for 95% of cases
P=0.95
...
Data are now pooled regardless chemistry and barcode-set sizes. In the graphs below, the number displayed in the right grey panel represents the size the randomly chosen subset of compatible combinations. We confirm that the greedy-descent search performs faster for small subsets of compatible combinations. For subsets larger than 30 compatible combinations, it seems that 50 iterations would be a reasonable number for an efficient and fast optimization by the greedy-exchange search.
These results are very similar to those obtained with the TruSeq small RNA kits from Illumina.

```{r echo = FALSE,  warning=FALSE, fig.width=11, fig.height=11, eval=T}
# for 95% of cases
P=0.95
...
```{r echo = FALSE,  warning=FALSE, message=FALSE, fig.width=11, fig.height=44, eval=T}
sim3_probaSmax %>% group_by(chemistry, barcode_number, mplex_level, nb_lane, algorithm) %>%
  filter(thrs_size_comb<nb_comp_combinations) %>% ungroup() %>%
  # filter(thrs_size_comb == 60) %>%
  mutate(isSmax=ifelse(abs(S_opt-S_max)<10^-10, T, F),
