...
The total number of combinations (compatible or not) is given by the binomial coefficient ${n \choose k}$ where $n$ is the number of barcodes in the dataset, referred to as the barcode-set size, and $k$ is the mutliplex level ($n$ and $k$ being positive integers).
...
We perform `r R` repetitions of the greedy search for the conditions (described in table 2.3.1 above). When the set of compatible combinations is large, the computation time, required to run these greedy algorithms on the full set of compatible combinations generated in step1, may become prohibitively large. To reduce the computation time, we apply the greedy search on a randomly generated subset of `r min(thrs_size_comb_for_GE)` $\leq N_{comp}\leq$ `r max(thrs_size_comb_for_GE)` compatible combinations, for a barcode set size comprized between `r min(L)` and `r max(L)`, and a multiplex level between `r min(N)` and `r max(N)`. Here, we want to know how the size of the subset influences the computation time for each of the greedy search algorithm.
...
#### Estimation of the execution time required for R iterations of a greedy search, with R chosen to ensure 95% confidence of obtaining a maximum entropy barcode selection
...
##### Case A: $n = 30$, $k = 4$, $a = 6$, $S_{max}$ is reached
...
##### Case B: $n = 30$, $k = 5$, $a = 6$, $S_{max}$ is **not** reached
...
##### Case C: $n = 12$, $k = 5$, $a = 6$, $S_{max}$ is reached
...
##### Case D: $n=12$, $k=4$, $a=6$, $S_{max}$ is **not** reached
...
### Estimation of the maximum execution time (R iterations) of a greedy search to obtain the max entropy in 95% of cases
