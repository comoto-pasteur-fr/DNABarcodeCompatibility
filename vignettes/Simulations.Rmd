...
* Step2: optimize the set of compatible barcodes to reduce redundancy between barcodes
    ...
    + Step2b: optimization by N iterations of a greedy search method


# Illumina barcode set from TruSeq small RNA kits
## Find compatible combinations barcodes: simulations relative to Step1 of the workflow
...
## Analyze barcode-set properties: set parameters for the simulations
...
## Optimize combinations: set parameters for simulations relative to Step2 of the workflow
...
#### Run simulations for step2a (without iteration)
...
For subsets larger than 30 compatible combinations, it seems that 50 iterations would be a reasonable number for an efficient and fast optimization by the greedy-exchange search.
...
#### Run simulations for step2b with iterations
...
## Optimize combinations: set parameters for the simulations
...
### Run simulations for the optimization without iteration
...
The results shown in the graphs below are qualitatively similar to those obtained with the Illumina data set. They are heterogeneous between conditions. However common trends emerge. First, in general, the greedy-exchange search performs the optimization faster over N iterations than the greedy-descent search, except for a few cases (see next point) where the subset of compatible combinations is smaller than 30. Second, for compatible-combination subsets larger than 120 (only tested for the greedy-exchange), the optimization requires less iterations in general and performs faster. Based on these observations, it seems reasonable to fix the compatible-combination subset size to 120.
...
Data are now pooled regardless chemistry and barcode-set sizes. From the 2 graphs below, we confirm that the greedy-descent search performs faster for small subsets of compatible combinations.
For subsets larger than 30 compatible combinations, it seems that 50 iterations would be a reasonable number for an efficient and fast optimization by the greedy-exchange search. These results are very similar to those obtained with the TruSeq small RNA kits from Illumina.
