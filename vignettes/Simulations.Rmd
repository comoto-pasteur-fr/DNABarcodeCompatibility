...
system.time({
  system(paste0("parallel -j",nb_cpu," Rscript /Users/retourna/DNABarcodeCompatibility/vignettes/sim_get_all_combinations.R {1} {2} {3} {4} {5} ::: ",paste(rep(N,R), collapse=" ")," ::: ",paste(L, collapse=" ")," ::: ",paste(C, collapse=" "), " ::: ", paste(outputfile1, collapse=""), " ::: ", paste(outputfile2, collapse="")))
})
...
N = 4
C = c(4)
thrs_size_comb=c(80, 120)#,160,320,640)
nb_lane=c(4)
...
# Run simulation using GNU parallel (Windows users need cygwin + Gnu parallel; MacOs: brew install parallel; Linux: apt-get install parallel)
system.time({
  system(paste0("parallel -j",nb_cpu," Rscript /Users/retourna/DNABarcodeCompatibility/vignettes/sim_optimize_combinations.R {1} {2} {3} {4} {5} {6} {7} {8} ::: ",paste(1:R, collapse=" ")," ::: ",paste(thrs_size_comb, collapse=" ")," ::: ",paste(inputfile, collapse=""), " ::: ", paste(nb_lane, collapse=""), " ::: ", paste(L, collapse=""), " ::: ", paste(4, collapse=""), " ::: ", paste(outputfile1, collapse="")))
})
...
probaSmax %>% mutate(isSmax=ifelse(S==Smax, T, F)) %>%
  group_by(barcode_number, chemistry, nb_lane, thrs_size_comb) %>%
  summarise(time.avg=mean(time), time.sd=sd(time), nb_comp_combinations=nb_comp_combinations[1], S_max=S_max[1], ProbaOfSmax=sum(isSmax)/length(S)) %>%
