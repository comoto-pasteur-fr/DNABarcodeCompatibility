      ...
      if (is.numeric(nb_lane)){
        ...
        if(nb_lane < nrow(combination_m)){
          if (nrow(combination_m) > thrs_size_comb){ 
            a_combination = DNABarcodeCompatibility:::recursive_entropy(combination_m[sample(1:nrow(combination_m),thrs_size_comb),],nb_lane)
            i = 0
...
print(paste(rep_number, nrow(combination_m), barcode_number, thrs_size_comb, chemistry, nb_lane), quote=F)


Smax = DNABarcodeCompatibility:::entropy_max(barcode_number, ncol(combination_m) * nb_lane)
elapsed <- as.numeric(system.time({opt_comb <- optimize_combinations(combination_m, nb_lane, index_number=barcode_number, thrs_size_comb=thrs_size_comb, max_iteration=0)}))[3]
S=DNABarcodeCompatibility:::entropy_result(opt_comb)
...
df <- data.frame(time=round(elapsed,3), nb_comp_combinations=nrow(combination_m), rep_number=rep_number, barcode_number=barcode_number, 
                 thrs_size_comb=thrs_size_comb, Smax, S, chemistry=chemistry, nb_lane=nb_lane)
